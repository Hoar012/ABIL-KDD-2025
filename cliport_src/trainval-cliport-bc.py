...
def main():
    ...
    from hacl.p.kfac.cliport import BCModel
    model = BCModel(args.task, None, predicates=PREDICATES)
    if args.load is not None:
        try:
    ...
    warm_up_ratio = 0.1 # warm up step
    ...
    for epoch in range(max_epoch):
        ...
        if (epoch + 1) % 10 == 0:
            ckpt_name = f'dumps/seed{args.seed}/bc-{args.task}-{load_id}-epoch{epoch}.pth'
            logger.critical('Saving checkpoint to "{}".'.format(ckpt_name))
            io.dump(ckpt_name, state_dict(model))
    

    ckpt_name = f'dumps/seed{args.seed}/bc-{args.task}-{load_id}.pth'
    logger.critical('Saving checkpoint to "{}".'.format(ckpt_name))
    io.dump(ckpt_name, state_dict(model))
