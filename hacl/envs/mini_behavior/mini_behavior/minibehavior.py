...
from hacl.envs.mini_behavior.mini_behavior.actions import Pickup, Drop, Toggle, Open, Close
import numpy as np
...
class MiniBehaviorEnv(MiniGridEnv):
    ...
    def iter_objects(self, ignore_walls = True) -> Iterator[Tuple[int, int, int, Tuple]]:
        k = 0
        objects = list()
        ...
        for j in range(self.height):
            for i in range(self.width):
                for k in range(3):
                    if self.grid.get(i,j)[k][0] is not None:
                        obj = self.grid.get(i,j)[k][0]
                        if ignore_walls and obj.type == 'wall':
                            continue
                        objects.append((i, j, k, obj))
                    
                    if self.grid.get(i,j)[k][1] is not None:
                        obj = self.grid.get(i,j)[k][1]
                        objects.append((i, j, k, obj))

        if self.carrying is not None and len(self.carrying) > 0:
            obj = list(self.carrying)[0]
            objects.append((-1, -1, -1, obj))

        objects.sort(key=lambda x: id(x[-1])) # keep the order of all objects
        return objects
