...
class AssemblingKits(Task):
    ...
    def reset(self, env):
        ...
        kit_id = env.add_object(kit_urdf, kit_pose, 'fixed')
        ...
        n_objects = 5
        if self.mode == 'train':
            obj_shapes = random.sample(list(self.train_set), n_objects)
        else:
            if self.homogeneous:
                obj_shapes = [np.random.choice(self.test_set)] * n_objects
            else:
                obj_shapes = random.sample(list(self.test_set), n_objects)
        ...
        for i in range(n_objects):
            ...
            kit_id = env.add_object(urdf, (pos, rot), 'fixed')
            self.objs_id[kit_id] = shape_names[obj_shapes[i]] + "container"
        ...
        for i in range(n_objects):
            ...
            self.objs_id[block_id] = shape_names[shape] + "block"
            if os.path.exists(urdf):
                os.remove(urdf)
