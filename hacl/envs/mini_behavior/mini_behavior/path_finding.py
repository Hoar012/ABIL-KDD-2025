...
from gym_minigrid.minigrid import MiniGridEnv, DIR_TO_VEC
        ...
        for action in NAVIGATION_ACTIONS:
            if action is MiniGridEnv.Actions.forward:
                ...
                for i in range(3):
                    for obj in cell[i]:
                        # print(obj)
                        #if obj is not None and not obj.can_overlap():
                        if obj is not None and obj.type!="door" and obj.type!="bed":
                            facing = True
                            break
...
def find_path(env: MiniGridEnv, target_pose: Tuple[int, int], target_dir: int):
    ...
    try:
        return run_heuristic_search(
            ...
            check_goal,
            get_priority,
            gen_get_navigation_successors(env),check_visited = False
        )[1]
    except RuntimeError:
        return None
