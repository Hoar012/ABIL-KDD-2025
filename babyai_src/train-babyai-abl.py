...
if args.env == 'minigrid':
    ...
    if args.task in ('goto', 'goto2', 'gotosingle', 'gotodoor'):
        ...
        def action_filter(a):
            return a.name in ['move', 'forward', 'lturn', 'rturn', 'toggle']
    elif args.task == 'unlock':
        def action_filter(a):
            return a.name in ['move', 'pickup', 'forward', 'lturn', 'rturn', 'toggle-tool']
    elif args.task == 'put':
        def action_filter(a):
            return a.name in ['move', 'pickup', 'forward', 'lturn', 'rturn', 'place']
    else:
...
def evaluate(domain, env, heuristic_model=None, visualize: bool = False):
    ...
    def planner(state, mission):
        ...
        return pds.heuristic_search_strips(
            ...
            state,
            mission,
            max_depth=15, max_expansions=2000,
...
def main():
    ...
    for i in jacinle.tqdm(range(1, args.iterations + 1)):
        ...
        while total_n < 32:
            ...
            #     print(predicate.feature_def.name)
            extra_monitors['time/data'] = time.time() - end; end = time.time()
            loss, monitors, output_dict = model(feed_dict={'states': states, 'actions': actions, 'dones': dones, 'goal_expr': goal}, task = args.task, count=i, forward_augmented=True)
        ...
        if args.save_interval > 0 and i % args.save_interval == 0:
            ckpt_name = f'dumps/{args.structure_mode}-{args.task}{args.seed}-load={load_id}-epoch={i}.pth'
    ...
    if args.evaluate:
        ...
        ckpt_name = f'dumps/{args.structure_mode}-{args.task}{args.seed}-load={load_id}.pth'
        logger.critical('Saving checkpoint to "{}".'.format(ckpt_name))
        io.dump(ckpt_name, state_dict(model))
