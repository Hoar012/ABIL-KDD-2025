...
from hacl.p.kfac.cliport import ABIL_BCModel

def get_filt_obj(model: ABIL_BCModel, state, predicates, task, action):
    if task in ["packing-shapes", "packing-5shapes"]:
        ...
        filt_item_pose = item_poses[torch.norm(item_poses-pose1, 1, dim=-1).argmin()]
        filt_item_image = item_images[torch.norm(item_poses-pose1, 1, dim=-1).argmin()]
        
        container_poses = state["item-pose"].tensor
        container_images = state["item-image"].tensor
        filt_container_pose = container_poses[torch.norm(container_poses-pose2, 1, dim=-1).argmin()]
        filt_container_image = container_images[torch.norm(container_poses-pose2, 1, dim=-1).argmin()]
        
        filt_obj_images = torch.stack([filt_item_image, filt_container_image])
        filt_obj_poses = torch.stack([filt_item_pose, filt_container_pose])
...
def main():
    ...
    model = ABIL_BCModel(args.task, domain_gr, predicates=PREDICATES)
    if args.load is not None:
        try:
