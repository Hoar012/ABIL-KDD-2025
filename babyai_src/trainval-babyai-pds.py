...
if args.env == 'minigrid':
    ...
    elif args.task == 'gen_pickup':
        def action_filter(a):
            return a.name in ['forward', 'lturn', 'rturn', 'pickup', 'toggle-tool']
    elif args.task == 'gen_open':
        def action_filter(a):
            return a.name in ['forward', 'lturn', 'rturn', 'pickup', 'toggle-tool']
...
def evaluate(domain, env, heuristic_model=None, visualize: bool = False):
    ...
    def planner(state, mission):
        ...
        return pds.heuristic_search_strips(
            ...
            state,
            mission,
            max_depth=20, max_expansions=5000,
...
def main():
    ...
    if args.evaluate:
        ...
        ckpt_name = f'dumps/seed{args.seed}/{args.structure_mode}-{args.task}-load={load_id}.pth'
        logger.critical('Saving checkpoint to "{}".'.format(ckpt_name))
        io.dump(ckpt_name, state_dict(model))
