...
class PutBlockInBowlUnseenColors(Task):
    ...
    def reset(self, env):
        ...
        for _ in range(n_bowls):
            ...
            self.objs_id[bowl_id] = selected_color_names[1] + "container"
            p.changeVisualShape(bowl_id, -1, rgbaColor=colors[1] + [1])
            bowl_poses.append(bowl_pose)
            
            self.objs.append(["bowl", bowl_id, selected_color_names[1]])
        ...
        for _ in range(n_blocks):
            ...
            self.objs_id[block_id] = "block"
            self.objs_id[block_id] = selected_color_names[0] + "block"
            p.changeVisualShape(block_id, -1, rgbaColor=colors[0] + [1])
            blocks.append((block_id, (0, None)))
            self.objs.append(["block", block_id, selected_color_names[0]])
        ...
        self.mission = {'pick':selected_color_names[0], 'place':selected_color_names[1]}
        self.goal_predicates = [selected_color_names[0], selected_color_names[1]]
        ...
        while n_distractors < max_distractors:
            ...
            if is_block:
                self.objs.append(["block", obj_id, distractor_color_names[n_distractors % len(colors)]])
                self.objs_id[obj_id] = distractor_color_names[n_distractors % len(colors)] + "block"
            else:
                self.objs.append(["bowl", obj_id, distractor_color_names[n_distractors % len(colors)]])
                self.objs_id[obj_id] = distractor_color_names[n_distractors % len(colors)] + "container"
...
class PutBlockInBowlComposedColors(Task):
    """Put Blocks in Bowl base class and task."""
    ...
    def reset(self, env):
        ...
        if self.mode == "train":
            while selected_color_names in [["red", "green"], ["yellow", "brown"], ["gray", "cyan"]]:
                selected_color_names = random.sample(all_color_names, 2)
                
        if self.mode == "test":
            # selected_color_names = random.choice([["red", "green"], ["yellow", "brown"], ["gray", "cyan"]])
            while selected_color_names in [["red", "green"], ["yellow", "brown"], ["gray", "cyan"]]:
                selected_color_names = random.sample(all_color_names, 2)
        colors = [utils.COLORS[cn] for cn in selected_color_names]
